    ...
    match &mut socket {
        InboundStream::TcpStream(stream) => {
            ...
            match classify_wire_protocol(&buf[..n]) {
                Ok(protocol) => Ok((protocol, socket)),
                err @ Err(DetectionError::Inconclusive)
                | err @ Err(DetectionError::MoreDataRequired(_)) => {
                    let real_buf = &buf[..n];
                    debug!(buffer = ?real_buf, err = ?err, "Detection was inconclusive on a TCP stream which is surprising; unknown protocol in the tunnel");
                    Ok((DetectedProtocol::Unknown, socket))
                }
            }
        }

        InboundStream::TlsStream(tokio_rustls::TlsStream::Server(server_stream)) => {
            let (_io, session) = server_stream.get_ref();

            Ok((tls::from_tls_session(session), socket))
        }

        InboundStream::TlsStream(tokio_rustls::TlsStream::Client(_client_stream)) => {
            // TODO: we could grab (_io, session), analyze the client session in the TLS protocol
            // analyzer and derive a similar `DetectedProtocol` thing.
            // We do not have any usecase for it as currently all Portail inbound streams are
            // server-initiated TLS streams for now.
            Err(std::io::Error::other(
                "protocol detection on client-intitiated TLS streams is not supported yet",
            ))
        }
    }
}
