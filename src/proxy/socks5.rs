...
use crate::{
    ...
    proxy::context::{RequestContext, TargetContext},
    state::State,
};
...
pub async fn connect_to_backend(
    ...
    if backend.identity_aware {
        ...
        let stream =
            crate::proxy::client_tls::connect_using_tls_auth(target_socket, domain, state.clone())
                .await?;
...
pub async fn serve_socks5<'s, S: AsyncRead + Unpin + AsyncWrite>(
    ...
    let mut target_context = TargetContext {
        initial_target: format!("{}", target_addr),
    ...
    ctx.acl_ctx.insert(
        "host",
        crate::acl::ast::ConcreteOperand::String(&target_context.initial_target),
    );
    ...
    match assessment.action {
        ...
        crate::acl::Action::Redirect(target) => {
            ...
            final_addr = TargetAddr::Ip(SocketAddr::from_str(&target).expect("IP address"));
        }
