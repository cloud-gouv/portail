...
use crate::{
    acl::{
        ast::{Action, ComparisonError, ConcreteOperand, Expression, Operand},
        hir,
    },
...
pub struct EvaluationContext<'s> {
    ...
    pub variables: HashMap<&'s str, ConcreteOperand<'s>>,
}
...
pub struct RequestAssessment<'s> {
    ...
    /// What are the contexts of the previous requires which still fullfills the `when` condition of
...
impl<'s> EvaluationContext<'s> {
    ...
    fn concretize_operand(
        ...
        match operand {
            Operand::Identifier(id) => {
                if self.variables.contains_key(id.as_str()) {
                    Ok(self.variables[id.as_str()].clone())
                } else {
                    return Err(InterpretationError::MissingVariableInContext(
                        Cow::Borrowed(id),
                    ));
                }
            }
            Operand::DottedIdentifier(id) => {
                let key = id.join(".");
                if self.variables.contains_key(key.as_str()) {
                    Ok(self.variables[key.as_str()].clone())
                } else {
                    return Err(InterpretationError::MissingVariableInContext(Cow::Owned(
                        key,
                    )));
                }
            }
            Operand::Set(s) => Ok(ConcreteOperand::Set(s.iter().map(|s| s.as_str()).collect())),
            Operand::Regex(r) => Ok(ConcreteOperand::Regex(r.clone())),
    ...
    /// failover.
    pub fn evaluate_routes(&self, rules: &'s hir::ACLHir) -> Vec<&'s BackendSettings> {
        ...
        for entry in &rules.routes {
            let active = if let Some(ref when_expr) = entry.when {
                self.evaluate_expression(&when_expr).unwrap()
            } else {
                true
            };
            ...
            // We fullfill when, let's look at the recommended routes.
    ...
    pub fn evaluate_request(
        ...
        for entry in &rules.policies {
            ...
            // We fullfill when and require, let's return the action.
...
#[cfg(test)]
mod tests {
    use crate::{acl::Action, config::BackendSettings};
    ...
    mod tests {
        ...
        fn test_policy_fail_wrong_type() {
            ...
            let mut ctx = EvaluationContext::empty();
    ...
    fn test_route_evaluation() {
        ...
        let ctx = EvaluationContext::empty();
        let routes = ctx.evaluate_routes(&hir);
