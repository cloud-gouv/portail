...
pub enum Action {
    Allow,
    // FIXME: reinforce the type posture here? PathBuf?
    Deny(Option<String>),
    // FIXME: reinforce the type posture here? URI?
    Redirect(String),
}
...
mod tests {
    ...
    fn test_equality_and_inequality() {
        ...
        assert_eq!(
            ConcreteOperand::compare(IPAddress(ip1), &Eq, IPAddress(ip1)).unwrap(),
            true
        );
        assert_eq!(
            ConcreteOperand::compare(IPAddress(ip1), &Ne, IPAddress(ip2)).unwrap(),
            true
        );
    ...
    fn test_regex_operator() {
        ...
        assert_eq!(
            ConcreteOperand::compare(
                ...
                &Comparator::Regex,
                ConcreteOperand::Regex(re.clone())
            )
            .unwrap(),
            true
        );
