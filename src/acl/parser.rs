...
fn action_statement(input: &mut &str) -> ModalResult<Action> {
    ...
    alt((
        ...
        (
            "deny",
            opt(preceded(" explain-template=", cut_err(string_literal))),
        )
            .map(|(_, tpl)| Action::Deny(tpl.map(|s| s.to_owned()))),
        preceded("redirect", cut_err(string_literal)).map(|s| Action::Redirect(s.to_owned())),
