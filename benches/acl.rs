...
use portail::{
    acl::{ast::ConcreteOperand, ast_to_hir, parser::parse_into_ast, ACLRules, EvaluationContext},
...
pub struct RandomEvaluationConfig {}

fn eval(host: &str, rules: ACLRules) {
    let mut eval_context = EvaluationContext::empty();
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let backend_setting = BackendSettings {
        ...
        target_address: std::net::SocketAddr::V4("1.1.1.1:443".parse().unwrap()),
    };
    ...
    for num_uris in &[1_000, 10_000, 100_000, 1_000_000] {
        ...
        c.bench_function(&format!("eval_blocklist_deny({})", num_uris), |b| {
            b.iter_batched(
                || rules.clone(),
                |rules| eval(black_box("blocked500.example.com"), black_box(rules)),
                criterion::BatchSize::SmallInput,
            );
        });
    }
